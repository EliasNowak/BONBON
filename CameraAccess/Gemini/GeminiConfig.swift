...
enum GeminiConfig {
  ...
  static var systemInstruction: String {
    if SettingsManager.shared.cookClawModeEnabled {
      return CookClawSystemPrompt.prompt
    }
    return SettingsManager.shared.geminiSystemPrompt
  }

  static var toolDeclarations: [[String: Any]] {
    if SettingsManager.shared.cookClawModeEnabled {
      return CookClawToolDeclarations.allDeclarations()
    }
