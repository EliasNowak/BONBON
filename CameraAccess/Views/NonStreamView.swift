...
struct NonStreamView: View {
  ...
  let onStartGlassesProcessing: () async -> Void
  @State private var sheetHeight: CGFloat = 300
  @State private var showSettings = false
  @State private var isStartingProcessing = false
  ...
  var body: some View {
    ZStack {
      ...
      VStack {
        ...
        HStack(spacing: 8) {
          ...
          Text(viewModel.knownDeviceCount > 0 ? "Glasses registered. Start to connect." : "Waiting for registered glasses")
            .font(.system(size: 14))
            .foregroundColor(.white.opacity(0.7))
        }
        .padding(.bottom, 12)
        .opacity(viewModel.hasActiveDevice ? 0.7 : 1)
        ...
        CustomButton(
          ...
          Task {
            ...
            await onStartGlassesProcessing()
    ...
    .onAppear {
      viewModel.refreshDeviceAvailability()
    }
